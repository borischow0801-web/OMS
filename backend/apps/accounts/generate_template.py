"""
生成用户导入Excel模板文件
使用Django环境运行：python manage.py shell < apps/accounts/generate_template.py
或者：cd backend && python manage.py shell -c "exec(open('apps/accounts/generate_template.py').read())"
"""
import os
import sys
import django
...
def generate_user_import_template():
    ...
    try:
        ...
        print("错误：请先安装openpyxl库：pip install openpyxl==3.1.2")
        return None
